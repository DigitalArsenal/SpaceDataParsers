...
* --------------------------------------------------------------------------- */
	...
	void polarm
		(
		double xp, double yp, double ttt, eOpt opt, double (&pm)[3][3] //* pm //std::vector< std::vector<double> > &pm
		)
	{
		//pm.resize(3);  // rows
...
* --------------------------------------------------------------------------- */
	...
	void framebias
		(
		char opt,
		double& term1, double& term2, double& term3, double (&fb)[3][3] //std::vector< std::vector<double> > &fb
		)
	{
		//fb.resize(3);  // rows
...
vector between the earth fixed (itrf) frame, and
...
* --------------------------------------------------------------------------- */
		...
		)
	{
		...
		double prec[3][3], nut[3][3], st[3][3], stdot[3][3], temp[3][3], tempmat[3][3], stp[3][3], nutp[3][3], precp[3][3];
		double psia, wa, epsa, chia, deltapsi, deltaeps, trueeps, meaneps,
...
* ----------------------------------------------------------------------------*/
		...
		)
	{
		...
		double prec[3][3], nut[3][3], nutp[3][3], precp[3][3];
		//std::vector< std::vector<double> > tempmat = std::vector< std::vector<double> >(3, std::vector<double>(3));
		double tempmat[3][3];
...
* ----------------------------------------------------------------------------*/
		...
		)
	{
		...
		double prec[3][3], nut[3][3], temp[3][3], tempmat[3][3], nutp[3][3], precp[3][3], eqep[3][3];
	...
	void teme_ecef_pos
		...
		edirection direct,
		double recef[3],
		double ttt, double jdut1, double xp, double yp, bool skipPolarMotion, int eqeterms
		)
	{
		double deg2rad, omega, gmstg;
		double st[3][3];
      		double pm[3][3];
                double pmp[3][3];
		double stp[3][3];
		double rpef[3], omgxr[3], omgxomgxr[3],
			omgxv[3], gmst;
		...
		if(!skipPolarMotion) coordFK5::polarm(xp, yp, ttt, e80, pm);
		...
		if (direct == eTo)
		{
			...
			else astMath::matvecmult(stp, rteme, recef);
		}
		else
		{
			if (!skipPolarMotion) {
			...
			else astMath::matvecmult(st, recef, rteme);
		}


	}  // procedure teme_ecef_pos

	void teme_ecef_matrix
		(
		double ttt, double jdut1, double xp, double yp, bool skipPolarMotion, int eqeterms,
                double (&polarMatrix)[3][3], double (&pefMatrix)[3][3]
		)
	{
		double deg2rad, omega, gmstg;
		double tmp[3][3];
		double omgxr[3], omgxomgxr[3],
			omgxv[3], gmst;
		...
		tmp[2][2] = 1.0;
	 	...
	 	astMath::mattrans(tmp, pefMatrix, 3, 3);
		if (!skipPolarMotion) {
			coordFK5::polarm(xp, yp, ttt, e80, tmp);
			astMath::mattrans(tmp, polarMatrix, 3, 3);
		}

	}  // procedure teme_ecef_matrix

	void teme_ecef_convert
		(
		double rteme[3],
		double recef[3],
		bool skipPolarMotion,
		double (&polarMatrix)[3][3], double (&pefMatrix)[3][3]
		)
	{
		...
		if (!skipPolarMotion) {
		        double rpef[3];
			astMath::matvecmult(pefMatrix, rteme, rpef);
			astMath::matvecmult(polarMatrix, rpef, recef);
		}
		else astMath::matvecmult(pefMatrix, rteme, recef);
	}  // procedure teme_ecef_convert
