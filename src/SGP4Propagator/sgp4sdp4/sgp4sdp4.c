...
SGP4(double tsince, tle_t *tle, vector_t *pos, vector_t *vel)
{
  ...
  r = a*(1-ecose);
  temp1 = 1/r;
  rdot = xke2*sqrt(a)*esine*temp1;
  rfdot = xke2*sqrt(pl)*temp1;
  temp2 = a*temp1;
  betal = sqrt(temp);
  temp3 = 1/(1+betal);
...
SDP4(double tsince, tle_t *tle, vector_t *pos, vector_t *vel)
{
  ...
  xmdf = deep_arg.xll;
  a = pow(xke2/deep_arg.xn,tothrd)*tempa*tempa;
