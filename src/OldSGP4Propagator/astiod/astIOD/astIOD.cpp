...
)
{
	...
	*  determine if the vectors are coplanar.
	...
	{
#ifdef _MSC_VER
		strcpy(error, sizeof(error), "not coplanar");
#else
	...
	*    the same direction, and non-zero.
	...
	{
#ifdef _MSC_VER
		strcpy(error, sizeof(error), "impossible");
#else
...
)
{
	...
	*   position vectors are reasonably close.  1.0 deg is only an estimate.
	...
	{
#ifdef _MSC_VER
		strcpy(error, sizeof(error), "angle > 1�");
#else
...
)
{
	...
	int err = fopen_s(&outfile, "../../astIOD/testastIOD.out", "w");
