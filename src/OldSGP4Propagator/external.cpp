...
extern "C"
{
  ...
  int sgp4Propagate(long *SatObjPointer, double epoch, bool temeToECEF, double * result, bool storeResult, bool storeResultTemeToECEF) {

    SatObj *nSatObj = (SatObj *)SatObjPointer;
    if (!(nSatObj->visible)) return -1; 

    double ro[THREE];
    double vo[THREE];
    double tt = ((epoch - nSatObj->unix_timestamp) / 1000) / 60;
    if (!storeResult) SGP4Funcs::sgp4_pos(nSatObj->satrec, tt, ro);
    else SGP4Funcs::sgp4(nSatObj->satrec, tt, ro, vo);
    if ((nSatObj->satrec).error > 0) {
	  ...
	  //printf("SGP4 error %u for SATNUM %lu at EPOCH %lf\n", (nSatObj->satrec).error, (nSatObj->satrec).satnum, epoch);
          if (result) memset(result, 0, sizeof(ro));
	  return (nSatObj->satrec).error;
    }
    if(nSatObj->array_index == 0){
    //printf("%d\n", temeToECEF);
    }
    if (temeToECEF)
    {
          ...
          */
          ...
          if (!storeResult || !storeResultTemeToECEF) {
                            coordFK5::teme_ecef_pos(ro,
                              eTo,
                              ecef, 
                              jcent,
                              jdut1,
			      ...
			      true,
                            ...
                            if (storeResultTemeToECEF) 
                            {
                              nSatObj->x = ecef[0];
                              nSatObj->y = ecef[1];
                              nSatObj->z = ecef[2];
                              nSatObj->vx = vcef[0]*1000;
                              nSatObj->vy = vcef[1]*1000;
                              nSatObj->vz = vcef[2]*1000;
                            }
          }

          if (result) memcpy(result, ecef, sizeof(ecef));
          if (!storeResult || storeResultTemeToECEF) return 0;
    }
    for (int i = 0; i < THREE; ++i)
    {
      ro[i] *= 1000;
  ...
  long *SatObjPointer = nullptr)
  {
    ...
    SatObj *nSatObj;
    if (SatObjPointer) nSatObj = (SatObj *) SatObjPointer;
    else {
	nSatObj = new SatObj();
	resized = true;
    }
  ...
  double *propagate(double epoch, bool temeToECEF, bool storeResult)
  {
    ...
    {
      sgp4Propagate((long *) *it, epoch, temeToECEF, (double *) (vec + index * THREE), storeResult, false); 
    }
  ...
  double *generateEphemeris(long *SatObjPointer, double start, double stop, int samplesPerPeriod, bool temeToECEF)
  {
    ...
    if(samplesPerPeriod == NULL) samplesPerPeriod = 90;
    if(temeToECEF == NULL) temeToECEF = true;
    ...
    if (!nSatObj->ephemeris_ptr) nSatObj->ephemeris_ptr = (double *)malloc(THREE * ephemerisLength * sizeof(double));
    for (int idx = 0; idx < ephemerisLength; ++idx, epoch += stepSize)
    {
      int status = sgp4Propagate(SatObjPointer, epoch, temeToECEF, (double *) nSatObj->ephemeris_ptr + idx * THREE, false, false);
    }
  ...
  double *getValue(long *SatObjPointer, double epoch, bool temeToECEF, bool storeResult, bool storeResultTemeToECEF)
  {
    //https://github.com/emscripten-core/emscripten/issues/3942
    if(temeToECEF == NULL) temeToECEF = true;
    sgp4Propagate(SatObjPointer, epoch, temeToECEF, value, storeResult, storeResultTemeToECEF);
    return value;
  }

  double *getValueInReferenceFrame(long *SatObjPointer, double epoch, bool temeToECEF, bool storeResult, bool storeResultTemeToECEF)
  {
    sgp4Propagate(SatObjPointer, epoch, temeToECEF, value, storeResult, storeResultTemeToECEF);
    return value;
  }

  double *getSatAzElRange(double *ecfEphemeris, int ephemerisSize, double *observerECFLatLonHeight) {
    // Need some sort of upper limit
    if (ephemerisSize > 1000000) return 0; 
  ...
  const char *describeObject()
  {
    ...
    if (!description.empty()) return description.c_str();
